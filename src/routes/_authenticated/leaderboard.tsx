    ...
    <div className="space-y-6">
      ...
      <GlassCard>
        ...
        <div className="mt-4 divide-y divide-white/10">
          ...
          {overallSorted.map((r) => (
            <div key={r.rank + r.team.name} className="flex items-center gap-4 py-3">
              ...
              <div className="mt-3 divide-y divide-white/10">
                {subs.length === 0 ? (
                  ...
                  subs.map((s: any, i: number) => (
                    <div key={s.event_id + s.team_id} className="flex items-center gap-4 py-2.5">
                      <span className={`w-8 text-center font-mono text-lg ${i === 0 ? "text-primary font-bold" : "text-white/50"}`}>
                        {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`}
                      </span>
                      <span className="text-white font-semibold">{(teamMap.get(s.team_id) ?? { name: "—" }).name}</span>
                      {teamMap.get(s.team_id)?.lead_name && (
                        <span className="text-sm text-white/40">— {teamMap.get(s.team_id)?.lead_name}</span>
                      )}
                    </div>
                  ))
                )}
